...
describe("The camelCaseObjectKeys function", () => {
  test("converts keys from kebab-case", () => {
    expect(camelCaseObjectKeys({"kebab-case": {name: "test"}})).toEqual({kebabCase: {name: "test"}})
  })

  test("converts keys from snake_case", () => {
    expect(camelCaseObjectKeys({"snake_case": {name: "test"}})).toEqual({snakeCase: {name: "test"}})
  })

  test("converts keys from TitleCase", () => {
    expect(camelCaseObjectKeys({TitleCase: {name: "test"}})).toEqual({titleCase: {name: "test"}})
  })

  test("converts all keys", () => {
    expect(camelCaseObjectKeys({
